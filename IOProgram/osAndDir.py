...
print('操作系统类型：', os.name, os.uname())  # 操作系统类型
...
print(os.path.split('/user/sfy/test.txt'), os.path.splitext('/user/sfy/test.txt'))
...
for f in os.listdir(pwd):
    ...
    print('%10d     %s  %s' % (fsize, mtime, flag))
...
def search(s):
    for f in os.listdir(pwd):
        if os.path.splitext(f)[0].find(s) >= 0:
            print(s, ':', pwd, r'/', f.title())
